...
public class Torch : MonoBehaviour
{
	...
	public void Fog()
    {
        ...
        {
            for (int j = (_y - vista); j <= (_y + vista); j++)
            {
                if (Mathf.Abs(i - _x) + Mathf.Abs (j - _y) == (vista))
                {
                    farCell.Add(new Vector2(i + 0.5f, j + 0.5f));
                }
            }
        }
        ...
        foreach (var cell in farCell)
        {
            ...
            List<GameObject> cellCast = new List<GameObject>();

            foreach (RaycastHit2D cella in hit)
            {
                cellCast.Add(cella.transform.gameObject);
            }

            foreach (var item in cellCast)
            {
